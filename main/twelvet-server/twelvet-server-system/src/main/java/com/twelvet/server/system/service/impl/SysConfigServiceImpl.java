...
import com.twelvet.server.system.mapper.SysConfigMapper;
...
public class SysConfigServiceImpl implements ISysConfigService {
    ...
    public String selectConfigByKey(String configKey) {
        ...
        if (TWTUtils.isNotEmpty(configValue)) {
            return configValue;
        }
        SysConfig config = new SysConfig();
        config.setConfigKey(configKey);
        SysConfig retConfig = configMapper.selectConfig(config);
        if (TWTUtils.isNotEmpty(retConfig)) {
    ...
    public String checkConfigKeyUnique(SysConfig config) {
        Long configId = TWTUtils.isEmpty(config.getConfigId()) ? -1L : config.getConfigId();
        SysConfig info = configMapper.checkConfigKeyUnique(config.getConfigKey());
        if (TWTUtils.isNotEmpty(info) && info.getConfigId().longValue() != configId.longValue()) {
